...
CREATE UNIQUE INDEX idx_unique_versions ON untracked((data->>'component'),(data->>'version'),(data->>'lang'));
...
DECLARE
  q text;
BEGIN
q = 'SELECT data ->> ''component'' AS component, data ->> ''version'' AS version FROM untracked WHERE data->''hashes'' @> ''[{"sha256": ' || to_json(hash) || '}]''';
...
DECLARE
  q text;
BEGIN
q = 'SELECT data ->> ''component'' AS component, data ->> ''version'' AS version FROM untracked WHERE data->''hashes'' @> ''[{"sha256": ' || to_json(hash) || '}]'' AND data->>''lang'' = ' || quote_literal(lang);
RETURN QUERY EXECUTE q;
END; $$
LANGUAGE 'plpgsql' STABLE STRICT LEAKPROOF PARALLEL SAFE SECURITY DEFINER;
